...
function CemberinAlani(cemberYaricapi) {
  return pi*cemberYaricapi*cemberYaricapi;
}
...
ucebolunenlerintoplami = ucetambolunenler.reduce((acc, x) => acc + x, 0);
...
for (let sayi in tekrarSayaci) {
  if (tekrarSayaci[sayi] > 1) {
    tekraredensayilar.push(`${sayi} sayısı ${tekrarSayaci[sayi]} kere tekrar edilmiştir.`);
  }
}
